...
type MemFSInput struct {
	Op	 uint8 // 0 => read, 1 => write(0), 2 => write(n) (append)
	Key	string
	Value []byte
}

type MemFSOutput struct {
	Value string
}

func MemFSModel() Model {
	return Model{
		Partition: func(history []Operation) [][]Operation {
			m := make(map[string][]Operation)
			for _, v := range history {
				key := v.Input.(MemFSInput).Key
				m[key] = append(m[key], v)
			}
			var ret [][]Operation
			for _, v := range m {
		...
		Step: func(state, input, output interface{}) (bool, interface{}) {
			inp := input.(MemFSInput)
			out := output.(MemFSOutput)
			st := state.([]byte)
			if inp.Op == 0 {
				// read(n)
				...
				// write(n) (append)
				//Eqiv to return true, (st + inp.Value)
				return true, append(st, inp.Value...)
			}
		},
		Equal: ShallowEqual,
	}
