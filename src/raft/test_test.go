...
func TestBackup2B(t *testing.T) {
	...
	//fmt.Println("pass%d", 5)

	// now another partitioned leader and one follower
	leader2 := cfg.checkOneLeader()
	//fmt.Println("pass%d", 6)
	...
	for i := 0; i < 50; i++ {
		...
		//cfg.rafts[leader2].Start(i + 200)
	}
	//fmt.Println("pass%d", 7)
	...
	cfg.connect(other)
	//fmt.Println("pass%d", 8)

	// lots of successful commands to new group.
	for i := 0; i < 50; i++ {
		cfg.one(rand.Int(), 3, true)
		//cfg.one(i+300, 3, true)
	}
	//fmt.Println("pass%d", 9)

	// now everyone
	for i := 0; i < servers; i++ {
		cfg.connect(i)
	}
	//fmt.Println("pass%d", 10)
	cfg.one(rand.Int(), servers, true)
	//cfg.one(400, servers, true)
	//fmt.Println("pass%d", 11)
...
loop:
	for try := 0; try < 5; try++ {
		...
		if total2-total1 > (iters+1+3)*3 {
			t.Fatalf("too many RPCs (%v) for %v UncompressedEntries\n", total2-total1, iters)
		}
...
func internalChurn(t *testing.T, unreliable bool) {
	...
	cfn := func(me int, ch chan []int) {
		//fmt.Printf("Creating concurrent client %d\n", me)
		var ret []int
		ret = nil
		defer func() { ch <- ret }()
		values := []int{}
