...
func (cfg *config) cleanup() {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	for i := 0; i < len(cfg.fileServers); i++ {
		if cfg.fileServers[i] != nil {
			cfg.fileServers[i].Kill()
		}
	}
	cfg.net.Cleanup()
	cfg.checkTimeout()
...
func (cfg *config) makeClient(to []int) *FSClerk {
	...
	ck := MakeFsClerk(random_handles(ends))
...
func (cfg *config) ShutdownServer(i int) {
	...
	kv := cfg.fileServers[i]
	if kv != nil {
		cfg.mu.Unlock()
		kv.Kill()
		cfg.mu.Lock()
		cfg.fileServers[i] = nil
	}
}
